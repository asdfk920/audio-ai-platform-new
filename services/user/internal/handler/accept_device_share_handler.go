...
// @Summary      接受设备共享
// @Description  接受其他用户共享的设备（单条 share_id/sn，或批量 devices/list）
// @Tags         设备分享
// @Accept       json
// @Produce      json
// @Param        body  body      types.DeviceShareAcceptReq  true  "接受设备共享请求"
// @Success      200  {object}  errorx.Response  "接受成功"
...
func AcceptDeviceShareHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(batch) > 0 {
			...
			httpx.WriteJsonCtx(r.Context(), w, http.StatusOK, errorx.SuccessData("接受设备共享成功", batchResp))
			return
		}
