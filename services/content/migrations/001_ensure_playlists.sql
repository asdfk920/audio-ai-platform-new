...
CREATE INDEX IF NOT EXISTS idx_playlists_source_id ON public.playlists(source_id);
CREATE INDEX IF NOT EXISTS idx_playlists_user_source ON public.playlists(user_id, source);

COMMENT ON TABLE public.playlists IS '用户歌单表';
...
CREATE INDEX IF NOT EXISTS idx_playlist_songs_content_id ON public.playlist_songs(content_id);

COMMENT ON TABLE public.playlist_songs IS '歌单与内容关联表';
