...
// devicePauseHandler 设备暂停指令处理器
// POST /api/device/cmd/pause
// 用途：用户通过 App 向设备下发暂停指令（需 JWT 鉴权）
// 流程：
...
func devicePauseHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return jwt.JwtMiddleware(svcCtx.Config.Auth.AccessSecret)(func(w http.ResponseWriter, r *http.Request) {
		...
		// 调用业务逻辑
		l := logic.NewDevicePauseLogic(r.Context(), svcCtx)
		resp, err := l.DevicePause(&req)
		if err != nil {
			httpx.WriteJson(w, http.StatusBadRequest, map[string]interface{}{
				"code": 400,
				"msg":  err.Error(),
