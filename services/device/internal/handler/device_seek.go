...
// POST /api/device/cmd/seek
// 拖动进度条：创建 seek 指令并通过 WebSocket 下发。
func deviceSeekHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return jwt.JwtMiddleware(svcCtx.Config.Auth.AccessSecret)(func(w http.ResponseWriter, r *http.Request) {
		...
		l := logic.NewDeviceSeekLogic(r.Context(), svcCtx)
		resp, err := l.DeviceSeek(&req)
		if err != nil {
			httpx.WriteJson(w, http.StatusBadRequest, map[string]interface{}{
				"code": 400,
				"msg":  err.Error(),
