...
// deviceVolumeDownHandler 设备音量减指令处理器
// POST /api/device/cmd/volume_down
// 用途：用户通过 App 向设备下发音量减指令（需 JWT 鉴权）
// 流程：
...
func deviceVolumeDownHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return jwt.JwtMiddleware(svcCtx.Config.Auth.AccessSecret)(func(w http.ResponseWriter, r *http.Request) {
		...
		// 解析请求体
		var req types.DeviceVolumeDownReq
		...
		// 调用业务逻辑
		l := logic.NewDeviceVolumeDownLogic(r.Context(), svcCtx)
		resp, err := l.DeviceVolumeDown(&req)
		if err != nil {
			httpx.WriteJson(w, http.StatusBadRequest, map[string]interface{}{
				"code": 400,
				"msg":  err.Error(),
