...
// deviceSetLoopHandler 设备设置循环播放指令处理器
// POST /api/device/cmd/set_loop
// 用途：用户通过 App 向设备下发设置循环播放指令（需 JWT 鉴权）
// 流程：
...
func deviceSetLoopHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return jwt.JwtMiddleware(svcCtx.Config.Auth.AccessSecret)(func(w http.ResponseWriter, r *http.Request) {
		...
		// 解析请求体
		var req types.DeviceSetLoopReq
		...
		// 调用业务逻辑
		l := logic.NewDeviceSetLoopLogic(r.Context(), svcCtx)
		resp, err := l.DeviceSetLoop(&req)
		if err != nil {
			httpx.WriteJson(w, http.StatusBadRequest, map[string]interface{}{
				"code": 400,
				"msg":  err.Error(),
