...
// deviceResumeHandler 设备继续播放指令处理器
// POST /api/device/cmd/resume
// 用途：用户通过 App 向设备下发继续播放指令（需 JWT 鉴权）
// 流程：
//   1. 接收 POST 请求，解析 Authorization 头获取用户 Token
//   2. 解析请求体 JSON 数据，获取 sn、action、volume 参数
//   3. 校验 Token 和参数格式
...
func deviceResumeHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return jwt.JwtMiddleware(svcCtx.Config.Auth.AccessSecret)(func(w http.ResponseWriter, r *http.Request) {
		...
		// 调用业务逻辑
		l := logic.NewDeviceResumeLogic(r.Context(), svcCtx)
		resp, err := l.DeviceResume(&req)
		if err != nil {
			httpx.WriteJson(w, http.StatusBadRequest, map[string]interface{}{
				"code": 400,
				"msg":  err.Error(),
