...
// deviceNextHandler 设备下一首指令处理器
// POST /api/device/cmd/next
// 用途：用户通过 App 向设备下发下一首指令（需 JWT 鉴权）
// 流程：
...
func deviceNextHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return jwt.JwtMiddleware(svcCtx.Config.Auth.AccessSecret)(func(w http.ResponseWriter, r *http.Request) {
		...
		// 调用业务逻辑
		l := logic.NewDeviceNextLogic(r.Context(), svcCtx)
		resp, err := l.DeviceNext(&req)
		if err != nil {
			httpx.WriteJson(w, http.StatusBadRequest, map[string]interface{}{
				"code": 400,
				"msg":  err.Error(),
