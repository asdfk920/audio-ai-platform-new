...
// DevicePrevLogic 设备上一首指令逻辑
// 处理用户通过 App 向设备下发上一首指令的业务逻辑
// 支持在线设备立即下发，离线设备缓存等待上线
// 包含用户身份验证、设备权限校验、参数校验等

type DevicePrevLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

// NewDevicePrevLogic 创建设备上一首指令逻辑实例
// 参数 ctx context.Context: 请求上下文
// 参数 svcCtx *svc.ServiceContext: 服务上下文
// 返回 *DevicePrevLogic: 设备上一首指令逻辑实例
func NewDevicePrevLogic(ctx context.Context, svcCtx *svc.ServiceContext) *DevicePrevLogic {
	return &DevicePrevLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
	}
}

// DevicePrev 下发设备上一首指令
// 接收上一首指令请求，验证用户权限和设备状态，下发上一首指令
// 参数 req *types.DevicePrevReq: 上一首指令请求
// 返回 *types.DevicePrevResp: 上一首指令响应
// 返回 error: 错误信息
func (l *DevicePrevLogic) DevicePrev(req *types.DevicePrevReq) (*types.DevicePrevResp, error) {
	// 1. 校验 Token 是否存在
	...
	// 2. 校验请求参数
	if err := validateDevicePrevReq(req); err != nil {
	...
	result, err := cmdSvc.CreateImmediateInstructionFromDesired(l.ctx, commandsvc.CreateImmediateInstructionInput{
		...
		Reason:          fmt.Sprintf("用户下发上一首指令: action=%s", action),
	})
	if err != nil {
		return nil, fmt.Errorf("创建上一首指令失败: %v", err)
	}

	// 7. 组装响应
	status, message := MapInstructionDispatchOutcome(result.Status, "上一首", "")

	logx.Infof("设备上一首指令已下发: user_id=%d, sn=%s, action=%s, instruction_id=%d, status=%s",
		userID, sn, action, result.InstructionID, status)

	return &types.DevicePrevResp{
...
// validateDevicePrevReq 校验上一首指令请求参数
// 参数 req *types.DevicePrevReq: 上一首指令请求
// 返回 error: 校验错误
func validateDevicePrevReq(req *types.DevicePrevReq) error {
	if req == nil {
	...
	action := strings.ToLower(strings.TrimSpace(req.Action))
	if action != "prev" {
		return fmt.Errorf("操作类型必须为 prev")
	}
