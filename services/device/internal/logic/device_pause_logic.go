...
// DevicePauseLogic 设备暂停指令逻辑
// 处理用户通过 App 向设备下发暂停指令的业务逻辑
// 支持在线设备立即下发，离线设备缓存等待上线
// 包含用户身份验证、设备权限校验、参数校验等

type DevicePauseLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

// NewDevicePauseLogic 创建设备暂停指令逻辑实例
// 参数 ctx context.Context: 请求上下文
// 参数 svcCtx *svc.ServiceContext: 服务上下文
// 返回 *DevicePauseLogic: 设备暂停指令逻辑实例
func NewDevicePauseLogic(ctx context.Context, svcCtx *svc.ServiceContext) *DevicePauseLogic {
	return &DevicePauseLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
	}
}

// DevicePause 下发设备暂停指令
// 接收暂停指令请求，验证用户权限和设备状态，下发暂停指令
// 参数 req *types.DevicePauseReq: 暂停指令请求
// 返回 *types.DevicePauseResp: 暂停指令响应
// 返回 error: 错误信息
func (l *DevicePauseLogic) DevicePause(req *types.DevicePauseReq) (*types.DevicePauseResp, error) {
	// 1. 校验 Token 是否存在
	...
	// 2. 校验请求参数
	if err := validateDevicePauseReq(req); err != nil {
	...
	result, err := cmdSvc.CreateImmediateInstructionFromDesired(l.ctx, commandsvc.CreateImmediateInstructionInput{
		...
		Reason:          fmt.Sprintf("用户下发暂停指令: action=%s", action),
	})
	if err != nil {
		return nil, fmt.Errorf("创建暂停指令失败: %v", err)
	}

	// 7. 组装响应
	status, message := MapInstructionDispatchOutcome(result.Status, "暂停", "")

	logx.Infof("设备暂停指令已下发: user_id=%d, sn=%s, action=%s, instruction_id=%d, status=%s",
		userID, sn, action, result.InstructionID, status)

	return &types.DevicePauseResp{
...
// validateDevicePauseReq 校验暂停指令请求参数
// 参数 req *types.DevicePauseReq: 暂停指令请求
// 返回 error: 校验错误
func validateDevicePauseReq(req *types.DevicePauseReq) error {
	if req == nil {
	...
	action := strings.ToLower(strings.TrimSpace(req.Action))
	if action != "pause" {
		return fmt.Errorf("操作类型必须为 pause")
	}
