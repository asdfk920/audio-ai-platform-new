...
// 处理用户通过 App 向设备下发音量加指令的业务逻辑
// 支持在线设备立即下发，离线设备缓存等待上线
// 包含用户身份验证、设备权限校验、参数校验、当前音量查询等

type DeviceVolumeUpLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

// NewDeviceVolumeUpLogic 创建设备音量加指令逻辑实例
// 参数 ctx context.Context: 请求上下文
// 参数 svcCtx *svc.ServiceContext: 服务上下文
// 返回 *DeviceVolumeUpLogic: 设备音量加指令逻辑实例
func NewDeviceVolumeUpLogic(ctx context.Context, svcCtx *svc.ServiceContext) *DeviceVolumeUpLogic {
	return &DeviceVolumeUpLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
	}
}

// DeviceVolumeUp 下发设备音量加指令
// 接收音量加指令请求，验证用户权限和设备状态，查询当前音量，计算目标音量，下发音量加指令
// 参数 req *types.DeviceVolumeUpReq: 音量加指令请求
// 返回 *types.DeviceVolumeUpResp: 音量加指令响应
// 返回 error: 错误信息
func (l *DeviceVolumeUpLogic) DeviceVolumeUp(req *types.DeviceVolumeUpReq) (*types.DeviceVolumeUpResp, error) {
	// 1. 校验 Token 是否存在
	...
	// 2. 校验请求参数
	if err := validateDeviceVolumeUpReq(req); err != nil {
	...
	result, err := cmdSvc.CreateImmediateInstructionFromDesired(l.ctx, commandsvc.CreateImmediateInstructionInput{
		...
		DeviceSN:        sn,
		UserID:          userID,
		CommandCode:     "volume_up",
		InstructionType: commandsvc.InstructionTypeManual,
		Params:          params,
		Operator:        fmt.Sprintf("user:%d", userID),
		Reason:          fmt.Sprintf("用户下发音量加指令: action=%s, step=%d, current=%d, target=%d", action, step, currentVolume, targetVolume),
	})
	if err != nil {
		return nil, fmt.Errorf("创建音量加指令失败: %v", err)
	}
	...
	return &types.DeviceVolumeUpResp{
...
// 返回 error: 查询错误
func (l *DeviceVolumeUpLogic) getCurrentVolume(sn string) (int, error) {
	// 1. 优先查询 Redis 缓存
...
// validateDeviceVolumeUpReq 校验音量加指令请求参数
// 参数 req *types.DeviceVolumeUpReq: 音量加指令请求
// 返回 error: 校验错误
func validateDeviceVolumeUpReq(req *types.DeviceVolumeUpReq) error {
	if req == nil {
	...
	action := strings.ToLower(strings.TrimSpace(req.Action))
	if action != "volume_up" {
		return fmt.Errorf("操作类型必须为 volume_up")
	}

	// 校验 step 参数（可选）
	if req.Step < 0 || req.Step > 20 {
		return fmt.Errorf("音量增量必须在 0-20 范围内")
	}
