...
func AuthCheckRole() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rk, _ := v["rolekey"].(string)
		// 检查权限：与历史 go-admin 一致，admin 跳过 Casbin；super_admin 为本平台冷启动超管 slug，须同等处理（否则 JWT 为 super_admin 时 Casbin 无 p 规则会全拒）
		if rbac.IsCasbinBypassRole(rk) {
			c.Next()
			return
		}
