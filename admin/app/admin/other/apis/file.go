...
func (e File) sysConfigValue(key string) string {
	if strings.TrimSpace(key) == "" || e.Orm == nil {
		return ""
	}
	type row struct {
	...
	_ = e.Orm.Table("sys_config").Select("config_value").
		Where("deleted_at IS NULL AND config_key = ?", strings.TrimSpace(key)).
		Limit(1).
		Scan(&r).Error
	return strings.TrimSpace(r.ConfigValue)
}

func (e File) uploadDriver(source string) string {
	if v := strings.TrimSpace(os.Getenv("UPLOAD_STORAGE_DRIVER")); v != "" {
		return strings.ToLower(v)
	}
	if v := e.sysConfigValue("upload_storage_driver"); v != "" {
		return strings.ToLower(v)
	}
	if source == "2" {
		return "oss"
...
func (e File) ossPublicURL(objectKey string) string {
...
func (e File) baseImg(c *gin.Context, fileResponse FileResponse, urlPrefix string) FileResponse {
	...
	if err := e.thirdUpload(source, fileName, base64File); err != nil {
		e.Error(200, errors.New(""), "上传第三方失败")
		return fileResponse
	}

	if e.uploadDriver(source) == "oss" {
		fileResponse.Path = "img/" + fileName
		fileResponse.FullPath = e.ossPublicURL("img/" + fileName)
	} else if source != "1" {
		// 其它第三方或兼容旧逻辑：仍返回本地静态路径
...
func (e File) multipleFile(c *gin.Context, urlPrefix string) []FileResponse {
	...
	for _, f := range files {
		...
		if err := e.thirdUpload(source, fileName, multipartFileName); err != nil {
...
func (e File) thirdUpload(source string, name string, path string) error {
	switch source {
	case "2":
		return e.ossUpload("img/"+name, path)
	case "3":
...
func (e File) ossUpload(name string, path string) error {
	// AK/SK 永远只读环境变量（避免泄露到 DB / 配置表）
	ak := strings.TrimSpace(os.Getenv("OSS_ACCESS_KEY_ID"))
	sk := strings.TrimSpace(os.Getenv("OSS_ACCESS_KEY_SECRET"))
	if ak == "" || sk == "" {
		return fmt.Errorf("oss env missing: OSS_ACCESS_KEY_ID/OSS_ACCESS_KEY_SECRET")
	}
	// endpoint/bucket 可由 env 覆盖，否则读 sys_config（便于你在后台更新 bucket）
	endpoint := strings.TrimSpace(os.Getenv("OSS_ENDPOINT"))
	if endpoint == "" {
		endpoint = e.sysConfigValue("upload_storage_oss_endpoint")
	}
	bucket := strings.TrimSpace(os.Getenv("OSS_BUCKET_NAME"))
	if bucket == "" {
		bucket = e.sysConfigValue("upload_storage_oss_bucket")
	}
	if endpoint == "" || bucket == "" {
		return fmt.Errorf("oss config missing: OSS_ENDPOINT/OSS_BUCKET_NAME (or sys_config upload_storage_oss_endpoint/upload_storage_oss_bucket)")
	}
	client := file_store.ALiYunOSS{}
	if err := client.Setup(endpoint, ak, sk, bucket); err != nil {
		return err
	}
...
func qiniuUpload(name string, path string) error {
	// 兼容占位：未配置七牛时返回明确错误
	return fmt.Errorf("qiniu upload not configured")
}
