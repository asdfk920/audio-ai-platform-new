...
import (
	"bytes"
	"fmt"
	"go-admin/app/admin/other/models/tools"
...
func (e Gen) Preview(c *gin.Context) {
	...
	table.TableId = id
	t1, err := template.ParseFiles("template/v4/dao.go.template")
	if err != nil {
		log.Error(err)
	...
	t6, err := template.ParseFiles("template/v4/dao.go.template")
	if err != nil {
		log.Error(err)
	...
	mp["template/dao.go.template"] = b1.String()
	...
	mp["template/dao.go.template"] = b6.String()
...
func (e Gen) NOActionsGen(c *gin.Context, tab tools.SysTables) {
	...
	t1, err := template.ParseFiles(basePath + "dao.go.template")
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf("model模版读取失败！错误详情：%s", err.Error()))
		return
	...
	t6, err := template.ParseFiles(basePath + "dao.go.template")
	if err != nil {
		log.Error(err)
	...
	_ = pkg.PathCreate("./app/" + tab.PackageName + "/service/dao/")
	...
	pkg.FileCreate(b6, "./app/"+tab.PackageName+"/service/dao/"+tab.TBName+".go")
