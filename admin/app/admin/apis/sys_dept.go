...
func (e SysDept) Get2Tree(c *gin.Context) {
	...
	// #region agent log
	if err != nil {
		debugSessionLog167bb8("H2", "sys_dept.go:Get2Tree", "dept_tree_failed", map[string]any{"err": err.Error()})
	} else {
		debugSessionLog167bb8("H2", "sys_dept.go:Get2Tree", "dept_tree_ok", map[string]any{"n": len(list)})
	}
	// #endregion
	if err != nil {
		e.Error(500, err, "查询失败")
